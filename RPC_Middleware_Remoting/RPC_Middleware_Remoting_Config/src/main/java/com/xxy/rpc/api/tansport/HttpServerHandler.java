...
import com.xxy.rpc.api.codec.CodecRegistry;
import com.xxy.rpc.api.codec.Encoder;
import com.xxy.rpc.api.request.CommandRequest;
import com.xxy.rpc.api.response.CommandResponse;
import com.xxy.rpc.api.utils.HttpCommandUtils;
...
public class HttpServerHandler extends SimpleChannelInboundHandler<Object> {
    ...
    protected void channelRead0(ChannelHandlerContext ctx, Object msg) throws Exception {
        ...
        try {
            ...
            if (StringUtils.isBlank(HttpCommandUtils.getTarget(request))) {
    ...
    private void writeResponse(CommandResponse response, ChannelHandlerContext ctx, boolean keepAlive)
        ...
        if (response.isSuccess()) {
            if (response.getResult() == null) {
                ...
                if (encoder == null) {
                    writeErrorResponse(INTERNAL_SERVER_ERROR.code(), SERVER_ERROR_MESSAGE, ctx);
                    LOGGER.warn("Error when encoding object",
            ...
            body = response.getException().getMessage().getBytes("UTF-8");
        }
