 ...
 */
package com.xxy.rpc.rpc.cluster.router.tag;

import com.xxy.rpc.common.config.configcenter.ConfigChangeType;
import com.xxy.rpc.common.config.configcenter.ConfigChangedEvent;
...
import com.xxy.rpc.rpc.Constants;
...
import com.xxy.rpc.rpc.cluster.router.tag.model.TagRuleParser;
import com.xxy.rpc.common.URL;
import com.xxy.rpc.rpc.cluster.router.tag.model.TagRouterRule;
...
public class TagRouter extends AbstractRouter implements ConfigurationListener {
    ...
    public <T> List<Invoker<T>> route(List<Invoker<T>> invokers, URL url, Invocation invocation) throws RpcException {
        ...
        String tag = StringUtils.isEmpty(invocation.getAttachment(CommonConstants.TAG_KEY)) ? url.getParameter(CommonConstants.TAG_KEY) :
                invocation.getAttachment(CommonConstants.TAG_KEY);
        ...
        if (StringUtils.isNotEmpty(tag)) {
            ...
            if (CollectionUtils.isNotEmpty(addresses)) {
                ...
                result = filterInvoker(invokers, invoker -> tag.equals(invoker.getUrl().getParameter(CommonConstants.TAG_KEY)));
            }
            ...
            // FAILOVER: return all Providers without any tags.
                ...
                return filterInvoker(tmp, invoker -> StringUtils.isEmpty(invoker.getUrl().getParameter(CommonConstants.TAG_KEY)));
            }
        } else {
            ...
            return filterInvoker(result, invoker -> {
                String localTag = invoker.getUrl().getParameter(CommonConstants.TAG_KEY);
    ...
    private <T> List<Invoker<T>> filterUsingStaticTag(List<Invoker<T>> invokers, URL url, Invocation invocation) {
        ...
        if (!StringUtils.isEmpty(tag)) {
            ...
            if (CollectionUtils.isEmpty(result) && !isForceUseTag(invocation)) {
                result = filterInvoker(invokers, invoker -> StringUtils.isEmpty(invoker.getUrl().getParameter(CommonConstants.TAG_KEY)));
            }
        } else {
            result = filterInvoker(invokers, invoker -> StringUtils.isEmpty(invoker.getUrl().getParameter(CommonConstants.TAG_KEY)));
        }
    ...
    private boolean isForceUseTag(Invocation invocation) {
        return Boolean.valueOf(invocation.getAttachment(Constants.FORCE_USE_TAG, url.getParameter(Constants.FORCE_USE_TAG, "false")));
    }
    ...
    private boolean checkAddressMatch(List<String> addresses, String host, int port) {
        for (String address : addresses) {
            try {
                ...
                if ((CommonConstants.ANYHOST_VALUE + ":" + port).equals(address)) {
                    return true;
                }
