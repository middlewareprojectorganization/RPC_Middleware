 ...
 */
package com.xxy.rpc.rpc.cluster.configurator.parser;
...
import com.xxy.rpc.rpc.cluster.configurator.parser.model.ConfigItem;
import com.xxy.rpc.rpc.cluster.configurator.parser.model.ConfiguratorConfig;
...
public class ConfigParser {
    ...
    private static List<URL> serviceItemToUrls(ConfigItem item, ConfiguratorConfig config) {
        ...
        addresses.forEach(addr -> {
            ...
            urlBuilder.append("&category=").append(RegistryConstants.DYNAMIC_CONFIGURATORS_CATEGORY);
    ...
    private static List<URL> appItemToUrls(ConfigItem item, ConfiguratorConfig config) {
        ...
        for (String addr : addresses) {
            ...
            for (String s : services) {
                ...
                urlBuilder.append("&category=").append(RegistryConstants.APP_DYNAMIC_CONFIGURATORS_CATEGORY);
                urlBuilder.append("&configVersion=").append(config.getConfigVersion());
    ...
    private static String toParameterString(ConfigItem item) {
        ...
        sb.append("category=");
        sb.append(RegistryConstants.DYNAMIC_CONFIGURATORS_CATEGORY);
    ...
    private static List<String> parseAddresses(ConfigItem item) {
        ...
        if (addresses.size() == 0) {
            addresses.add(CommonConstants.ANYHOST_VALUE);
        }
        return addresses;
    }
}
