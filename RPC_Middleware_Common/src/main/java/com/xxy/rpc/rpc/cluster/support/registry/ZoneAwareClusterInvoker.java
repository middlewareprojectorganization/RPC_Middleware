 ...
 */
package com.xxy.rpc.rpc.cluster.support.registry;
...
import com.xxy.rpc.rpc.cluster.LoadBalance;
...
public class ZoneAwareClusterInvoker<T> extends AbstractClusterInvoker<T> {
    ...
    public Result doInvoke(Invocation invocation, final List<Invoker<T>> invokers, LoadBalance loadbalance) throws RpcException {
        ...
        for (Invoker<T> invoker : invokers) {
            if (invoker.isAvailable() && invoker.getUrl().getParameter(RegistryConstants.REGISTRY_KEY + "." + CommonConstants.PREFERRED_KEY, false)) {
                return invoker.invoke(invocation);
            }
        }

        // providers in the registry with the same
        String zone = (String) invocation.getAttachment(RegistryConstants.REGISTRY_ZONE);
        if (StringUtils.isNotEmpty(zone)) {
            for (Invoker<T> invoker : invokers) {
                if (invoker.isAvailable() && zone.equals(invoker.getUrl().getParameter(RegistryConstants.REGISTRY_KEY + "." + RegistryConstants.ZONE_KEY))) {
                    return invoker.invoke(invocation);
                }
            }
            String force = (String) invocation.getAttachment(RegistryConstants.REGISTRY_ZONE_FORCE);
