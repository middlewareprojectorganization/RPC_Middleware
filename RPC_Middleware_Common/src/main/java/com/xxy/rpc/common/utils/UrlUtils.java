...
import com.xxy.rpc.common.constants.RemotingConstants;
...
public class UrlUtils {
    ...
    public static URL parseURL(String address, Map<String, String> defaults) {
        ...
        if (address.contains("://") || address.contains(URL_PARAM_STARTING_SYMBOL)) {
            ...
            String[] addresses = CommonConstants.COMMA_SPLIT_PATTERN.split(address);
            url = addresses[0];
        ...
        String defaultProtocol = defaults == null ? null : defaults.get(CommonConstants.PROTOCOL_KEY);
        if (defaultProtocol == null || defaultProtocol.length() == 0) {
            defaultProtocol = CommonConstants.DUBBO_PROTOCOL;
        }
        String defaultUsername = defaults == null ? null : defaults.get(CommonConstants.USERNAME_KEY);
        String defaultPassword = defaults == null ? null : defaults.get(CommonConstants.PASSWORD_KEY);
        int defaultPort = StringUtils.parseInteger(defaults == null ? null : defaults.get(CommonConstants.PORT_KEY));
        String defaultPath = defaults == null ? null : defaults.get(CommonConstants.PATH_KEY);
        Map<String, String> defaultParameters = defaults == null ? null : new HashMap<String, String>(defaults);
        if (defaultParameters != null) {
            defaultParameters.remove(CommonConstants.PROTOCOL_KEY);
            defaultParameters.remove(CommonConstants.USERNAME_KEY);
            defaultParameters.remove(CommonConstants.PASSWORD_KEY);
            defaultParameters.remove(CommonConstants.HOST_KEY);
            defaultParameters.remove(CommonConstants.PORT_KEY);
            defaultParameters.remove(CommonConstants.PATH_KEY);
        }
    ...
    public static List<URL> parseURLs(String address, Map<String, String> defaults) {
        ...
        String[] addresses = CommonConstants.REGISTRY_SPLIT_PATTERN.split(address);
    ...
    public static URL getEmptyUrl(String service, String category) {
        ...
        return URL.valueOf(RegistryConstants.EMPTY_PROTOCOL + "://0.0.0.0/" + service + URL_PARAM_STARTING_SYMBOL
                + RegistryConstants.CATEGORY_KEY + "=" + category
                + (group == null ? "" : "&" + CommonConstants.GROUP_KEY + "=" + group)
                + (version == null ? "" : "&" + CommonConstants.VERSION_KEY + "=" + version));
    }

    public static boolean isMatchCategory(String category, String categories) {
        if (categories == null || categories.length() == 0) {
            return RegistryConstants.DEFAULT_CATEGORY.equals(category);
        } else if (categories.contains(CommonConstants.ANY_VALUE)) {
            return true;
        } else if (categories.contains(CommonConstants.REMOVE_VALUE_PREFIX)) {
    ...
    public static boolean isMatch(URL consumerUrl, URL providerUrl) {
        ...
        if (!(CommonConstants.ANY_VALUE.equals(consumerInterface)
                || CommonConstants.ANY_VALUE.equals(providerInterface)
                || StringUtils.isEquals(consumerInterface, providerInterface))) {
            return false;
        }

        if (!isMatchCategory(providerUrl.getParameter(RegistryConstants.CATEGORY_KEY, RegistryConstants.DEFAULT_CATEGORY),
                consumerUrl.getParameter(RegistryConstants.CATEGORY_KEY, RegistryConstants.DEFAULT_CATEGORY))) {
            return false;
        }
    ...
    public static boolean isServiceKeyMatch(URL pattern, URL value) {
        return pattern.getParameter(CommonConstants.INTERFACE_KEY).equals(
                value.getParameter(CommonConstants.INTERFACE_KEY))
                && isItemMatch(pattern.getParameter(CommonConstants.GROUP_KEY),
                value.getParameter(CommonConstants.GROUP_KEY))
                && isItemMatch(pattern.getParameter(CommonConstants.VERSION_KEY),
                value.getParameter(CommonConstants.VERSION_KEY));
    }
    ...
    public static boolean isConfigurator(URL url) {
        return RegistryConstants.OVERRIDE_PROTOCOL.equals(url.getProtocol()) ||
                RegistryConstants.CONFIGURATORS_CATEGORY.equals(url.getParameter(RegistryConstants.CATEGORY_KEY, RegistryConstants.DEFAULT_CATEGORY));
    }

    public static boolean isRoute(URL url) {
        return RegistryConstants.ROUTE_PROTOCOL.equals(url.getProtocol()) ||
                RegistryConstants.ROUTERS_CATEGORY.equals(url.getParameter(RegistryConstants.CATEGORY_KEY, RegistryConstants.DEFAULT_CATEGORY));
    }

    public static boolean isProvider(URL url) {
        return !RegistryConstants.OVERRIDE_PROTOCOL.equals(url.getProtocol()) &&
                !RegistryConstants.ROUTE_PROTOCOL.equals(url.getProtocol()) &&
                RegistryConstants.PROVIDERS_CATEGORY.equals(url.getParameter(RegistryConstants.CATEGORY_KEY, RegistryConstants.PROVIDERS_CATEGORY));
    }

    public static boolean isRegistry(URL url) {
        return RegistryConstants.REGISTRY_PROTOCOL.equals(url.getProtocol()) || RegistryConstants.SERVICE_REGISTRY_PROTOCOL.equalsIgnoreCase(url.getProtocol());
    }
    ...
    public static boolean isServiceDiscoveryRegistryType(Map<String, String> parameters) {
        ...
        return RegistryConstants.SERVICE_REGISTRY_TYPE.equals(parameters.get(RegistryConstants.REGISTRY_TYPE_KEY));
    }
